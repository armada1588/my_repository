...
/*
//インプットしたフォントのヘッダ情報をfontheader fh で受け取る。内容を読み込んでフォントポインタを作成する関数
//unsigned char **p = mainで宣言したフォントのポインタのポインタ
//unsigned char **p = ポインタの初期位置を記録する
//int *fx = 説明むずいけどフォントファイルのx軸の倍率
//return fontheder = あとでいっぱい使うけど，必要なのはfhのX,Yサイズなので本当は別にいらないけど便利だから返す
*/
